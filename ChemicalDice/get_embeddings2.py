...
def main(X1, X2, X3 ,X4 ,X5 , X6, embedding_sizes, model_state):




    # X6=graph_result2.drop(target_column,axis=1)
    ...
    data = MyDataset(np.array(X1), np.array(X2), np.array(X3), np.array(X4), np.array(X5), np.array(X6), np.array([-1] * len(result_list)))
    data_loader = DataLoader(data, batch_size = batch_size, shuffle=False, worker_init_fn=worker_init_fn) 
    ...
    latent_space_dims = [X1_dim, X2_dim, X3_dim, X4_dim, X5_dim, X6_dim]
    embed_dim=8192
    print('original size=', embed_dim)
    print('user size=', user_embed_size)
    for choice in [1, 2]:
        net_cdi = ChemicalDiceIntegrator(latent_space_dims=latent_space_dims, embedding_dim=embed_dim, choice=choice).to(device)
        net_cdi.load_state_dict(torch.load(f"../weights/trainAE_{dataset_name}_explainability_{choice}/{embed_dim}_cdi.pt"))
        finetune_cdi = FineTuneChemicalDiceIntegrator(net_cdi, user_embed_size, embed_dim).to(device)
        finetune_cdi, loss_train, loss_val, _, _ = finetune_AE(finetune_cdi, dataset_name, user_embed_size, choice, data_loader, data_loader, 500, True)
        finetune_cdi.load_state_dict(torch.load(f"../weights/finetune_AE_{dataset_name}_explainability_{choice}/{user_embed_size}_ft_cdi.pt"))
        ...
        writeEmbeddingsIntoFile('../embeddings/', f'FineTune_{dataset_name}_embed_{user_embed_size}_{choice}.csv', ids, embeddings)